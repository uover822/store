...
function jsonfile_store(options) {
  ...
  function wrangle(jsonstr) {
    let c, i = 0, l = 0, njsonstr = ''
    do {
      c = String.fromCharCode(jsonstr[i])
      if (c == '{')
        ++l
      else
      if (c == '}')
  ...
  function do_load(args, qent, filepath, cb) {
    Fs.readFile(filepath, function(err, jsonstr) {
      if (err && 'ENOENT' === err.code) {
        ...
        if (jsonstr.length && !jsonstr.includes('undefined:1')) {
          var njsonstr = wrangle(jsonstr)
          var data = JSON.parse(njsonstr, function(key, val) {
            if (_.isString(val)) {
                ...
                do_load(args, qent, filepath, function(err, fent) {
                  if (good(args, err, cb)) {
                    ...
                    if(Array.isArray(q)) {
                      ...
                      for (var p in q) {
                        if (typeof fent != 'undefined' && fent != null && typeof p != 'undefined' &&
                            p != null && !~p.indexOf('$') && q[p] !== fent[p]) {
                          return nextfile(i + 1)
                        }
                      }
                      entlist.push(fent)
